...
class AppointmentPaymentRefundCommand extends Task
{
    protected function configure()
    {
        $this->setName('task:appointment:payment:refund')
            ->addArgument(
                'appointment_id',
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        if (!$paymentService->refundTransaction($appointment)) {
            throw new Exception(sprintf("Appointment #%s payment error: %s ", $appointmentId, $paymentService->getLastError()));
        }


        $output->writeln(sprintf("Appointment's #%s payment succesfully refunded", $appointmentId));
    }
}
